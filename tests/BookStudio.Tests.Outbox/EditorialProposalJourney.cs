...
internal static class EditorialProposalJourney
{
    public static async Task RunAsync(string workspaceRoot)
    {
        ...
        var rejectionDiscoveryId = Guid.NewGuid();

        await using (var discovery = new SqliteDiscoverySessionStore(factory))
        {
            await CompleteDiscoveryAsync(discovery, workspace, projectId, discoveryId, "primary", now);
            await CompleteDiscoveryAsync(discovery, workspace, projectId, rejectionDiscoveryId, "rejection", now.AddSeconds(30));
        }
        ...
        await using (var store = new SqliteEditorialProposalStore(factory))
        {
            ...
            var rejectionEvidence = new[] { new ProposalEvidenceReference("DISCOVERY_ANSWER", "premise", $"discovery:{rejectionDiscoveryId:D}:premise:v1") };
            var rejectedDraft = draftProposal with { ProposalId = rejectedId, DiscoverySessionId = rejectionDiscoveryId, Evidence = rejectionEvidence, RequestFingerprint = "reject-create" };
            ...
            var redraft = await store.ReviseAsync(new EditorialProposalRevisionCommand(Guid.NewGuid(), workspace, rejectedId, 1, Content("Narrowed"), rejectionEvidence, "editor", "Address rejection", "reject-revise"), now.AddMinutes(15));
    ...
    private static async Task CompleteDiscoveryAsync(SqliteDiscoverySessionStore store, string workspace, Guid projectId, Guid discoveryId, string suffix, DateTimeOffset at)
    {
        var draft = new DiscoverySessionDraft(discoveryId, projectId, workspace, "1.0.0", new[] { new DiscoveryQuestion("premise", 1, DiscoveryQuestionType.Text, true, "Premise") }, $"proposal-discovery-{suffix}");
        await store.CreateAsync(draft, at);
        await store.AnswerAsync(new DiscoveryAnswerCommand(Guid.NewGuid(), workspace, discoveryId, "premise", "\"A durable editorial system\"", "editor", $"proposal-answer-{suffix}"), at.AddMinutes(1));
        await store.CompleteAsync(new DiscoveryCompleteCommand(Guid.NewGuid(), workspace, discoveryId, "editor", $"proposal-discovery-complete-{suffix}"), at.AddMinutes(2));
    }
