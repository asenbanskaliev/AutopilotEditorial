...
public sealed class SqliteLegalRiskStore : ILegalRiskStore, IAsyncDisposable
{
    ...
    private static void Outbox(SqliteConnection c,SqliteTransaction tx,LegalRiskCase i,string type,DateTimeOffset at)=>Exec(c,tx,"INSERT INTO outbox_messages(message_id,event_type,payload_json,occurred_at_utc,status,attempt_count,next_attempt_at_utc) VALUES($id,$t,$p,$at,'Pending',0,$at)",("$id",i.MessageId!.Value.ToString("D")),("$t",type),("$p",JsonSerializer.Serialize(new{i.WorkspaceId,i.CaseId,i.SubjectId})),("$at",at.ToString("O")));
