...
CREATE TABLE developmental_findings (
    workspace_id TEXT NOT NULL,
    review_id TEXT NOT NULL,
    ...
    FOREIGN KEY (workspace_id, review_id)
        REFERENCES developmental_reviews(workspace_id, review_id)
        ON DELETE CASCADE
);
...
CREATE TABLE developmental_receipts (
    workspace_id TEXT NOT NULL,
...
CREATE INDEX ix_developmental_reviews_authority
ON developmental_reviews(workspace_id, project_id, editorial_plan_id);

CREATE INDEX ix_developmental_history_review
ON developmental_history(workspace_id, review_id, history_id);
