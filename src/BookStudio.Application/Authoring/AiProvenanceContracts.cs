...
public interface IAiProvenanceStore
{
...
public sealed record AiProvenanceDraft(Guid RecordId, Guid ProjectId, string WorkspaceId, Guid RightsLicenseCaseId, long ExpectedRightsRevision, string ExpectedRightsDigest, Guid AssetId, AssetKind AssetKind, string AssetReference, string AssetDigest, int AssetVersion, string Actor, string SnapshotJson, string RequestFingerprint);
public sealed record AiProvenanceEvaluateCommand(Guid RequestId, string WorkspaceId, Guid RecordId, long ExpectedRevision, AiProvenanceClassification Classification, string? Provider, string? Model, string? ModelVersion, string PromptReference, string HumanTransformations, decimal? AiContributionPercent, IReadOnlyList<AiDisclosureDraft> Disclosures, string Evidence, string Actor, string RequestFingerprint);
...
public sealed record AiDisclosureDraft(Guid DisclosureId, string Channel, string Locale, string Format, string PolicyVersion, string Text, bool PolicyCompliant, string Evidence);
public sealed record AiDisclosure(Guid DisclosureId, string Channel, string Locale, string Format, string PolicyVersion, string Text, bool PolicyCompliant, string Evidence);

public sealed record AiProvenanceRecord(Guid RecordId, Guid ProjectId, string WorkspaceId, Guid RightsLicenseCaseId, long ExpectedRightsRevision, string ExpectedRightsDigest, Guid AssetId, AssetKind AssetKind, string AssetReference, string AssetDigest, int AssetVersion, string Actor, string SnapshotJson, long Revision, AiProvenanceStatus Status, AiProvenanceClassification? Classification, string? Provider, string? Model, string? ModelVersion, string? PromptReference, string? HumanTransformations, decimal? AiContributionPercent, IReadOnlyList<AiDisclosure> Disclosures, string? Evidence, AiProvenanceDecision? Decision, string? DecisionReason, Guid? MessageId, DateTimeOffset CreatedAtUtc, DateTimeOffset UpdatedAtUtc);
